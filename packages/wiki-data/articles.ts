...
import type { UniversalSection } from '@dds/types/section';
...
export interface WikiArticle extends UniversalSection {
  meta: {
    wiki: WikiMeta;
  };
}
    ...
    content: {
      ...
      paragraphs: [
        ...
        },
        {
          ...
          description:
            'Skeptics note that abundance has historically failed to reach the people who need it most. Distributional questions — who owns the models, who benefits from cheap energy, who is governed by which [[Governance Protocols|protocols]] — remain the fulcrum of the debate. The wiki treats these critiques as load-bearing, not peripheral.',
        },
      ],
    },
    display: {
...
export function listFeaturedArticles(): WikiArticle[] {
  return articles;
}
 ...
 */
export function listArticlesSortedByDate(
  deriveWikiMeta: (article: WikiArticle) => any
): Array<{ article: WikiArticle; meta: any }> {
  return articles
 ...
 */
export function listAllTags(deriveWikiMeta: (article: WikiArticle) => any): string[] {
