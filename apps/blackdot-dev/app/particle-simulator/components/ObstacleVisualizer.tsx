...
export function ObstacleVisualizer({
  ...
  const [loadedSDF, setLoadedSDF] = useState<((pos: THREE.Vector3) => number) | null>(null);
  ...
  const handleDroneLoad = useCallback((model: THREE.Group, sdf: (pos: THREE.Vector3) => number) => {
    setModelLoaded(true);
    setLoadedSDF(() => sdf);
