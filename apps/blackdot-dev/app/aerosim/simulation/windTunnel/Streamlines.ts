...
export class Streamlines {
  ...
  static createStreamlineTubes(
    ...
    for (const streamline of streamlines) {
      ...
      const tubeGeometry = new THREE.TubeGeometry(
        ...
        64, // segments along curve - increased for smoother curves
        tubeRadius,
        radialSegments,
        false // closed
      );
  ...
  /**
   * Create tube geometry with velocity-based coloring
   * Colors streamlines from blue (slow) to red (fast) based on velocity magnitude
   */
  static createColoredStreamlineTubes(
    streamlines: THREE.Vector3[][],
    field: VelocityField,
    tubeRadius: number = 0.5,
    ...
    for (const streamline of streamlines) {
      ...
      // Calculate vertex colors based on velocity
      ...
      for (let i = 0; i < positions.length; i += 3) {
        ...
        const velocity = field.sampleVelocity(pos);
        const speed = velocity.length();

        // Color: blue (slow) to red (fast)
        const normalized = Math.min(speed / maxSpeed, 1.0);
        const hue = (1.0 - normalized) * 0.67; // 0.67 = blue, 0 = red
