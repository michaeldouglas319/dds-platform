...
/**
 * Interface for velocity field sampling (works with both VelocityField and FluidSimulator)
 */
 ...
 * Now supports both VelocityField and FluidSimulator
 */
export class ParticleAdvection {
  ...
  static advectRK4(
    ...
    velocitySource: VelocityField | FluidSimulator | VelocitySampler,
    dt: number
  ): THREE.Vector3 {
    // k1 = V(p)
    const k1 = velocitySource.sampleVelocity(position);

    // k2 = V(p + 0.5*dt*k1)
    const p2 = position.clone().addScaledVector(k1, 0.5 * dt);
    const k2 = velocitySource.sampleVelocity(p2);

    // k3 = V(p + 0.5*dt*k2)
    const p3 = position.clone().addScaledVector(k2, 0.5 * dt);
    const k3 = velocitySource.sampleVelocity(p3);

    // k4 = V(p + dt*k3)
    const p4 = position.clone().addScaledVector(k3, dt);
    const k4 = velocitySource.sampleVelocity(p4);
  ...
  static advectEuler(
    ...
    const velocity = velocitySource.sampleVelocity(position);
  ...
  static advectBatch(
    ...
    return positions.map((pos) => advectFn(pos, velocitySource, dt));
  }
}
