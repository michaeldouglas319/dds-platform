...
import { MULTI_SOURCE_ORBIT_CONFIG, COLLISION_BUBBLE_CONFIG } from './config/multi-source-v2-enhanced.config';

// NEW V2 ENHANCEMENTS: Momentum handoff, orientation, visualization
import { MomentumHandoffController } from './lib/momentumHandoff';
import { calculateForwardFacingPolarLocked } from './lib/orientationController';
import { calculateSourceTrajectory } from './lib/trajectoryCalculator';
import { AutoOrbitZone } from './lib/orbitVisualization';

/**
 * Curved Takeoff → Orbit Showcase (V2 Enhanced)
 *
 * V2 Enhancements:
 * - Smooth momentum handoff at transitions (no jitter)
 * - Forward-polar-lock orientation (no roll/banking)
 * - Auto-calculated entry/exit points (standardization)
 * - 3D donut orbit visualization (shows actual volume)
 *
 * V1 Core Preserved:
 * - Orbital mechanics (lines 929-970) - UNTOUCHED
 * - Collision system - UNTOUCHED
 * - Exit attraction - UNTOUCHED
 *
...
interface Particle {
  ...
  // === V2 ENHANCEMENTS ===
  handoffActive?: boolean;         // NEW: Track if momentum handoff is blending
...
export function CurvedTakeoffOrbit({
  ...
  // === V2 ENHANCEMENT: Momentum Handoff Controller ===
  const handoffController = useMemo(() => {
    if (!config.momentumHandoff) return null;
    return new MomentumHandoffController({
      entryBlendDuration: config.momentumHandoff.entry.blendDuration,
      exitBlendDuration: config.momentumHandoff.exit.blendDuration,
      easingMode: config.momentumHandoff.entry.easingMode,
      speedMatchMode: config.momentumHandoff.entry.speedMatchMode,
      maxAcceleration: config.momentumHandoff.entry.maxAcceleration,
    });
  }, [config.momentumHandoff]);
  ...
  useFrame((state, delta) => {
    ...
    particlesRef.current.forEach((particle, idx) => {
      ...
      if (particle.phase === 'takeoff') {
        ...
        if (progress >= 1) {
          // === V2 ENHANCEMENT: Entry Handoff (Takeoff → Orbit) ===
          if (handoffController && config.momentumHandoff?.entry.blendDuration > 0) {
            // Calculate current velocity from curve tangent
            const curveExitVelocity = curve.getTangentAt(1.0).normalize().multiplyScalar(source.orbitEntryVelocity);

            // Calculate target orbit tangent velocity
            const orbitTangentVelocity = new THREE.Vector3(
              -Math.sin(source.orbitEntryAngle),
              0,
              Math.cos(source.orbitEntryAngle)
            ).multiplyScalar(source.orbitEntryVelocity);

            // Start momentum blend
            handoffController.startEntryHandoff(
              parseInt(particle.id),
              curveExitVelocity,
              orbitTangentVelocity
            );
            particle.handoffActive = true;

            if (config.momentumHandoff.logTransitions) {
              console.log(`🚀 Entry handoff started: ${particle.id} (${source.id})`);
            }
          }
      ...
      // ========================================================================
        ...
        // === Calculate vertical distribution (needed for both handoff and normal orbit) ===
        ...
        // === V2 ENHANCEMENT: Apply Blended Velocity During Handoff ===
        if (particle.handoffActive && handoffController && handoffController.isBlending(parseInt(particle.id))) {
          const blendedVelocity = handoffController.updateHandoff(parseInt(particle.id), delta);

          if (blendedVelocity) {
            // Apply blended velocity to position
            particle.position.add(blendedVelocity.clone().multiplyScalar(delta));

            // Update orbit angle based on actual position (stay synchronized)
            const dx = particle.position.x - config.orbit.center.x;
            const dz = particle.position.z - config.orbit.center.z;
            particle.orbitAngle = Math.atan2(dz, dx);
          }
        } else {
          // === ORIGINAL V1 ORBIT CODE (Untouched) ===
          ...
          // Clear handoff flag if it was active
          if (particle.handoffActive) {
            particle.handoffActive = false;
          }
        }
        ...
        // === V2 ENHANCEMENT: Forward-Polar-Lock Orientation Option ===
        if (config.orientation?.mode === 'forward-polar-lock') {
          // Use forward-polar-lock (no roll/banking)
          const velocity = tempVec3; // Velocity vector already calculated above
          const worldUp = config.orientation.polarLock?.worldUp || new THREE.Vector3(0, 1, 0);

          const targetQuat = calculateForwardFacingPolarLocked(
            velocity,
            worldUp,
            source?.modelOrientation?.rotationOffset ?
              new THREE.Euler(...source.modelOrientation.rotationOffset) : undefined
          );

          // Smooth rotation toward target
          const rotationSpeed = config.orientation.rotationSpeed || Math.PI * 2;
          particle.quaternion.rotateTowards(targetQuat, rotationSpeed * delta);
        } else {
          // === ORIGINAL V1 ORIENTATION (Default) ===
        ...
        if (particle.landingEligible) {
          ...
          if (particle.distanceToExit < exitZoneConfig.radius) {
            // === V2 ENHANCEMENT: Exit Handoff (Orbit → Landing) ===
            if (handoffController && config.momentumHandoff?.exit.blendDuration > 0) {
              // Calculate current orbit velocity
              const orbitVelocity = new THREE.Vector3(
                -Math.sin(particle.orbitAngle),
                0,
                Math.cos(particle.orbitAngle)
              ).multiplyScalar(particle.orbitSpeed);

              // Calculate landing curve entry velocity (approximate from first segment)
              const landingCurve = new THREE.CatmullRomCurve3(config.landingWaypoints);
              const landingTangent = landingCurve.getTangentAt(0);
              const landingSpeed = config.landingDuration > 0 ? landingCurve.getLength() / config.landingDuration : 0.3;
              const landingEntryVelocity = landingTangent.normalize().multiplyScalar(landingSpeed);

              // Start momentum blend
              handoffController.startExitHandoff(
                parseInt(particle.id),
                orbitVelocity,
                landingEntryVelocity
              );
              particle.handoffActive = true;

              if (config.momentumHandoff.logTransitions) {
                console.log(`🛬 Exit handoff started: ${particle.id}`);
              }
            }
  ...
  return (
    <group>
      ...
      {/* ===================================================================== */}
      {/* V2 ENHANCEMENT: 3D Donut Orbit Visualization */}
      {/* ===================================================================== */}
      {config.visual?.orbitZone?.enabled && (
        <AutoOrbitZone
          config={{
            orbit: {
            ...
            sources: config.sources.map(s => ({
              orbit: {
                ...
                verticalOscillation: {
                  enabled: true,
                  amplitude: config.orbitHeightVariation * 2.0,
                },
              },
            })),
          }}
          visualMode={config.visual.orbitZone.mode || 'wireframe'}
          color={config.visual.orbitZone.color || '#4444ff'}
          opacity={config.visual.orbitZone.opacity || 0.3}
          visible={true}
        />
      )}
